 ...
 *
 * After tools like Bash, Read, Grep, Glob, WebFetch, WebSearch execute,
 * this hook checks if the output is large and adds a compressed summary
 * as additionalContext so Claude can quickly reference key information.
 *
 * v1.3.0: Adaptive thresholds, WebFetch/WebSearch support, improved
 * Bash/Read fallback, per-tool stats tracking.
 *
...
// Per-tool base thresholds (tokens) — lower = compress sooner
...
/**
 * Adaptive threshold: lower thresholds for specific tools + compress
 * more aggressively as the session grows larger.
 */
export function getThreshold(toolName: string, stats: SessionStats): number {
  const base = PER_TOOL_THRESHOLD[toolName] ?? BASE_THRESHOLD;
  const total = stats.totalTokensBefore;
  const multiplier = total > 500000 ? 0.33 : total > 300000 ? 0.5 : total > 100000 ? 0.75 : 1.0;
...
export function summarizeBash(text: string, command: string): string {
  ...
  if (/(error|warning|failed)/i.test(text) && !/(git log|git status|ls\s)/i.test(command)) {
  ...
  // git log — count commits, show last summary
  if (/^commit [0-9a-f]{40}/m.test(text) || /git\s+log/i.test(command)) {
    ...
    if (commitMatches && commitMatches.length > 0) {
      ...
      // Extract first commit message (line after "Date:" or first non-empty non-header line)
  ...
  // git status — count modified/untracked, list file names
  if (/git\s+status/i.test(command)) {
    const modified = lines.filter((l) => /^\s*(modified|renamed|deleted):/i.test(l.trim()));
    const untracked = lines.filter(
      (l) => l.trim().length > 0 && !l.startsWith('#') && /^\t[^\s]/.test(l),
    );
    const staged = lines.filter((l) => /^\s*(new file|modified|deleted):/i.test(l.trim()));
    const fileNames = [...modified, ...staged]
      .map((l) => l.trim().replace(/^(modified|renamed|deleted|new file):\s*/i, ''))
      .slice(0, 15);
    return `[cortex] Git status: ${modified.length} modified, ${staged.length} staged, ${untracked.length} untracked. Files: ${fileNames.join(', ')}`;
  }

  // ls/find output — group by file extension
  ...
  // JSON responses: show structure (lower threshold — 1000 chars)
  const trimmed = text.trim();
  if ((trimmed.startsWith('{') || trimmed.startsWith('[')) && trimmed.length > 1000) {
    try {
  ...
  // Improved generic fallback: head + tail + error detection
  const head = lines.slice(0, 3).join(' | ');
  const tail = lines.length > 6 ? ` ... last: ${lines.slice(-2).join(' | ')}` : '';
  const errorCount = lines.filter((l) => /error|exception|fail/i.test(l)).length;
  const errorNote = errorCount > 0 ? ` (${errorCount} error lines detected)` : '';
  return `[cortex] Command \`${command}\` produced ${lines.length} lines.${errorNote} First 3: ${head}${tail}`;
}

/**
 * Summarize large file read with content-type awareness
 */
export function summarizeFileRead(text: string, filePath: string): string {
  ...
  // JSON config files: show top-level keys
  if (ext === 'json') {
    const trimmed = text.trim();
    try {
      const parsed = JSON.parse(trimmed);
  ...
  // YAML config files: show top-level keys
  if (ext === 'yaml' || ext === 'yml') {
    const topKeys = lines.filter((l) => /^[a-zA-Z_][\w-]*:/.test(l)).map((l) => l.split(':')[0]);
    if (topKeys.length > 0) {
...
/**
 * Summarize WebFetch output — extract headings as outline
 */
export function summarizeWebFetch(text: string, url: string): string {
  const lines = text.split('\n');

  // Extract markdown headings
  const headings = lines.filter((l) => /^#{1,4}\s/.test(l)).map((l) => l.trim().substring(0, 80));
  ...
  if (codeBlockCount > 0) parts.push(`${Math.floor(codeBlockCount)} code blocks.`);

  return parts.join(' ');
}

/**
 * Summarize WebSearch output — extract top result titles
 */
export function summarizeWebSearch(text: string, query: string): string {
  const lines = text.split('\n');

  // Extract result titles (lines that look like titles — markdown links or bold text)
  const titles: string[] = [];
  for (const line of lines) {
...
async function main(): Promise<void> {
  try {
    ...
    // Adaptive threshold based on tool + session size
    ...
    if (summary) {
      ...
      if (!stats.perTool[toolName]) {
        stats.perTool[toolName] = { compressed: 0, tokensBefore: 0, tokensAfter: 0 };
      }
